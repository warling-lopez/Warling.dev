...
const FormulariosDigitalesPro = () => {
    ...
    const [templates, setTemplates] = useState([
        { id: 1, name: 'Contrato de Servicios.pdf', date: '01/08/2025', fields: [] },
        { id: 2, name: 'Consentimiento Informado.pdf', date: '28/07/2025', fields: [] }
    ]);
    ...
    useEffect(() => {
        if (currentView === 'editor' && currentTemplate) {
            renderPdf(currentTemplate.fileUrl || 'https://raw.githubusercontent.com/mozilla/pdf.js-sample-files/master/tracemonkey.pdf');
            initDraggables();
        }
    ...
    useEffect(() => {
        if (showSignatureModal && signatureCanvasRef.current && window.SignaturePad) {
    ...
    return (
        <div id="app" className="flex flex-col h-screen font-sans bg-gray-50 antialiased">
                ...
                <main id="dashboard-view" className="flex-1 overflow-y-auto">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <input type="file" id="pdf-upload-input" className="hidden" onChange={handlePdfUpload} accept="application/pdf" />
                        </div>
                            ...
                            <ul className="divide-y divide-gray-200 mt-4">
                                {templates.map(template => (
                                    <li key={template.id} className="py-4 flex items-center justify-between">
                                        <div className="flex items-center space-x-4">
                                            <i data-lucide="file-text" className="h-6 w-6 text-gray-400"></i>
                                            <div>
                                        ...
                                        <div className="flex items-center space-x-3">
                                            <button onClick={() => editTemplate(template.id)} className="text-blue-600 hover:text-blue-800 text-sm font-medium flex items-center">
                                                <i data-lucide="edit" className="mr-1 h-4 w-4"></i>Editar
                                            </button>
                                            <button onClick={() => openShareModal(template)} className="text-green-600 hover:text-green-800 text-sm font-medium flex items-center">
                                                <i data-lucide="share-2" className="mr-1 h-4 w-4"></i>Compartir
                                            </button>
                    ...
                    <div className="flex-1 flex overflow-hidden">
                        <aside className="w-64 bg-gray-50 p-6 border-r border-gray-200 overflow-y-auto">
                            ...
                            <div className="space-y-3">
                                <div onClick={() => addField('text')} className="flex items-center p-3 rounded-lg bg-white border border-gray-200 cursor-pointer hover:bg-gray-100 hover:border-blue-500 transition-all">
                                    <i data-lucide="type" className="h-5 w-5 text-gray-500 mr-3"></i>
                                ...
                                <div onClick={() => addField('date')} className="flex items-center p-3 rounded-lg bg-white border border-gray-200 cursor-pointer hover:bg-gray-100 hover:border-blue-500 transition-all">
                                    <i data-lucide="calendar" className="h-5 w-5 text-gray-500 mr-3"></i>
                                ...
                                <div onClick={() => addField('checkbox')} className="flex items-center p-3 rounded-lg bg-white border border-gray-200 cursor-pointer hover:bg-gray-100 hover:border-blue-500 transition-all">
                                    <i data-lucide="check-square" className="h-5 w-5 text-gray-500 mr-3"></i>
                                ...
                                <div onClick={() => addField('signature')} className="flex items-center p-3 rounded-lg bg-white border border-gray-200 cursor-pointer hover:bg-gray-100 hover:border-blue-500 transition-all">
                                    <i data-lucide="pen-tool" className="h-5 w-5 text-gray-500 mr-3"></i>
                        ...
                        <main className="flex-1 bg-gray-100 p-8 overflow-auto flex justify-center">
                            <div id="pdf-container" className="relative">
                                <canvas ref={pdfCanvasRef} id="pdf-render" style={{ boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05)' }}></canvas>
                                {currentTemplate.fields.map((field, index) => (
                                    <div key={field.id} id={`field-${field.id}`} className="draggable-field"
                                        style={{ top: `${field.y}px`, left: `${field.x}px`, width: `${field.width}px`, height: `${field.height}px` }}>
                                        <div className="handle"><i data-lucide="move" className="h-3 w-3"></i></div>
                ...
                <div id="filler-view" className="flex-1 flex flex-col bg-gray-100">
                    ...
                    <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 flex justify-center">
                        <div id="filler-pdf-container" className="relative bg-white p-4 rounded-lg shadow-lg">
                            <img src="https://placehold.co/800x1131/ffffff/000000?text=Formulario+de+Ejemplo" className="max-w-full h-auto" alt="Formulario de ejemplo" />
                            {currentTemplate.fields.map(field => (
                                <div key={field.id} style={{ position: 'absolute', top: `${field.y}px`, left: `${field.x}px`, width: `${field.width}px`, height: `${field.height}px` }}>
                                    ...
                                    {field.type === 'signature' && (
                                        <div onClick={openSignatureModal} className="w-full h-full bg-yellow-100 border border-yellow-400 rounded-md cursor-pointer flex items-center justify-center text-gray-500 text-sm">
                                            <i data-lucide="pen-tool" className="mr-2 h-4 w-4"></i> Haz clic para firmar
                                        </div>
                                    )}
                                </div>
                            ))}
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-30">
                    <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setShowShareModal(false)}><i data-lucide="x" className="h-6 w-6 text-gray-500"></i></button>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 bg-gray-100 p-2 rounded-md">
                            <input type="text" readOnly value={shareLink} ref={shareLinkInputRef} className="bg-transparent w-full text-gray-700 focus:outline-none" />
            ...
            {showSignatureModal && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-40">
                    <div className="bg-white p-6 rounded-lg shadow-xl max-w-lg w-full">
                        ...
                        <canvas ref={signatureCanvasRef} id="signature-canvas" className="border border-gray-300 rounded-md w-full h-48"></canvas>
